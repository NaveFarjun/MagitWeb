...
public class Repository {
    ...
    private PullRequestManager m_PullRequestManager;
    ...
    private void InitMaps() {
        ...
        this.m_PullRequestManager = new PullRequestManager();
        //this.m_PullRequests = new HashMap<>();
        this.m_ActiveBranch = new Branch();
        DeltasListReset();
    }

    private void DeltasListReset() {
    ...
    private List<String> getChangedList() {
    ...
    /*public Map<Integer,PullRequest> getPullRequestsMap(){return m_PullRequests;}*/
    ...
    public void Push(User user, Repository rrRepo) throws CommitException, RepositoryDoesntTrackAfterOtherRepositoryException, IOException, ParseException, RemoteTrackingBranchException, OpenChangesException {
        IsRepositoryHasAtLeastOneCommit();
    ...
    public void resetBranch(Branch branch,SHA1 sha1,User user) throws OpenChangesException, CommitException, ParseException, IOException {
        if (getActiveBranch()==branch){
            ResetHeadBranch(sha1,user);
        }
        else{
            branch.setCommitSH1(sha1);

            FileUtils.WriteToFile(sha1.getSh1(), GetLocation()+BRANCHES_FOLDER + branch.getName() + ".txt");
        }
    }

    public void Pull(User user, Repository rrRepo) throws RepositoryDoesnotExistException, RepositoryDoesntTrackAfterOtherRepositoryException, ParseException, CommitException, IOException, OpenChangesException, BranchDoesNotExistException {
        if (getRRLocation() == null) {
            throw new RepositoryDoesntTrackAfterOtherRepositoryException("Current function available only on cloned repositories ");
        }
        if (HasOpenChanges(user)) {
    ...
    public Branch CreateNewRemoteTrackingBranch(String name, RemoteBranch remoteBranch) throws RepositoryDoesnotExistException, CommitException, BranchNameIsAllreadyExistException, IOException {
        if (getCommitMap().isEmpty()) {
            throw new CommitException("you don't have any commits so you can't make new branch");
        }
        if (getBranchesMap().containsKey(name)) {
...
*/
    public List<CommitDetails> ShowActiveBranchHistory(String branchName) throws CommitException {
        //Commit currentCommit=getCommitFromMapCommit(getActiveBranch().getCommitSH1());
        Commit currentCommit=getCommitFromMapCommit(getBranchesMap().get(branchName).getCommitSH1());
        List<CommitDetails> list=new ArrayList<>();
        ShowActiveBranchHistoryRec(currentCommit,list);
        for(CommitDetails commitDetails:list){
            list.removeIf(v->v.getM_CommitSHA1().equals(commitDetails.getM_CommitSHA1()) && v!=commitDetails);
        }
        return list;

    }

    private void ShowActiveBranchHistoryRec(Commit currentCommit, List<CommitDetails> list) {
        CommitDetails currentCommitDetails=new CommitDetails(currentCommit);
        list.add(currentCommitDetails);
        for(SHA1 prevCommitSha1:currentCommit.getPrevCommits()){
            Commit prevCommit=getCommitFromMapCommit(prevCommitSha1);
            ShowActiveBranchHistoryRec(prevCommit,list);
        }
    }
    ...
    public void pushLocalBranchToRemoteBranch(Branch branchToPush, Repository RR) throws RepositoryDoesntTrackAfterOtherRepositoryException, IOException, ParseException, BranchNameIsAllreadyExistException, CommitException, RepositoryDoesnotExistException, HeadBranchDeletedExcption, BranchDoesNotExistException, BranchFileDoesNotExistInFolderException {
        if (getRRLocation() == null) {
            throw new RepositoryDoesntTrackAfterOtherRepositoryException("Current function available only on cloned repositories ");
        }
        else if(branchToPush instanceof RemoteBranch || branchToPush instanceof RemoteTrackingBranch) {
        ...
        else
        {
            ...
            RR.CreateNewBranch(branchToPush.getName(),firstCommitOnBranch.MakeSH1());
            RemoteBranch remoteBranch = createRemoteBranch(branchToPush.getName(),branchToPush.getCommitSH1(),RR.getName());
            Branch bra=CreateNewRemoteTrackingBranch(branchToPush.getName(),remoteBranch);
             setActiveBranch(bra);
        }
    }
