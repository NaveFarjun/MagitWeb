...
public class RepositoryManager {
    ...
    public void ChangeRepository(Repository repository) throws IOException, ParseException {
    ...
    public List<CommitDetails> ShowActiveBranchHistory(String branchName) throws CommitException, RepositoryDoesnotExistException {
        IsCurrentRepositoryInitialize();
        return m_currentRepository.ShowActiveBranchHistory(branchName);
    }


    public void resetBranch(Branch branch,SHA1 sha1) throws CommitException, OpenChangesException, ParseException, IOException {
        m_currentRepository.resetBranch(branch,sha1,m_User);
    }
    ...
    public List<FileDetails> ShowAllCommitFiles (SHA1 commitSha1) throws RepositoryDoesnotExistException, ParseException {
        IsCurrentRepositoryInitialize();
        return m_currentRepository.ShowAllCommitFiles(commitSha1);
    }
    ...
    private void LoadBranches(MagitRepository magitRepository, Map<String, SHA1> commitIDToCommitSha1) throws IOException, BranchDoesNotExistException, ParseException, BranchIsAllReadyOnWCException, CheckoutToRemoteBranchException, CommitException, OpenChangesException {
        ...
        if (m_currentRepository.getBranchesMap().size()!=0 && m_currentRepository.getBranchesMap().get(magitRepository.getMagitBranches().getHead()).getCommitSH1() != null) {
   ...
   public void LoadXML(String location) throws BranchIsAllReadyOnWCException, IOException, BranchDoesNotExistException, ParseException, RepositoryAllreadyExistException, CheckoutToRemoteBranchException, CommitException, OpenChangesException {
    ...
    public void Pull(Repository rrRepo) throws RepositoryDoesnotExistException, RepositoryDoesntTrackAfterOtherRepositoryException, ParseException, CommitException, IOException, OpenChangesException, BranchDoesNotExistException {
        IsCurrentRepositoryInitialize();
        m_currentRepository.Pull(m_User, rrRepo);
    }

    public void Push(Repository rrRepo) throws CommitException, RepositoryDoesntTrackAfterOtherRepositoryException, IOException, ParseException, RemoteTrackingBranchException, RepositoryDoesnotExistException, OpenChangesException {
        IsRepositoryHasAtLeastOneCommit();
        m_currentRepository.Push(m_User,rrRepo);
    }
    ...
    public void pushLocalBranchToRemoteBranch(Branch b, Repository RR) throws IOException, CommitException, BranchNameIsAllreadyExistException, BranchFileDoesNotExistInFolderException, RepositoryDoesnotExistException, RepositoryDoesntTrackAfterOtherRepositoryException, ParseException, BranchDoesNotExistException, HeadBranchDeletedExcption {
        m_currentRepository.pushLocalBranchToRemoteBranch(b, RR);
    }

    public Map<Integer,PullRequest> getAllPullRequests(){
       return m_currentRepository.getPullRequestManager().getPullRequestMap();
    }

    public  void solvePullRequest(int id, PullRequestStatus pullRequestStatus) throws OpenChangesException, CommitException, ParseException, IOException {
       PullRequest pullRequest=getAllPullRequests().get(id);
       if (pullRequestStatus==PullRequestStatus.ACCEPTED) {
           Branch base = m_currentRepository.getBranchesMap().get(pullRequest.getBase().getName());
           Branch tatget = m_currentRepository.getBranchesMap().get(pullRequest.getTarget().getName());
           resetBranch(base, tatget.getCommitSH1());
       }
       m_currentRepository.getPullRequestManager().SolvePR(id,pullRequestStatus);
    }
}
